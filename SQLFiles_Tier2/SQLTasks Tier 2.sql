...
FROM Facilities
WHERE membercost > 0.0
AND membercost < 0.2* monthlymaintenace;
...
SELECT DISTINCT m.surname AS LAST, m.firstname AS FIRST, f.name AS FACILITY
FROM Bookings as b
...
s = '''SELECT f.name AS FACILITY, m.surname || ' ' || m.firstname AS LAST_FIRST, 
    CASE
...
WITH sub AS (
    SELECT concat(f.name, m.firstname, m.surname) AS NAME, 
	CASE
		WHEN m.memid = 0 THEN b.slots * f.guestcost 
		ELSE b.slots * f.membercost 
	END AS COST
